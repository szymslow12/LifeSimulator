...
public class LifeForm extends Square implements Runnable {
    ...
    public LifeForm(int posX, int poxY, AppController appController) {
        super("LIFE_FORM", 60, posX, poxY, appController);
    }

    private void splitIntoTwoForm() {
        int x = getPosition().getX();
        int y = getPosition().getY();
        (new Thread(new LifeForm(x,y, getAppController()))).start();
    }
